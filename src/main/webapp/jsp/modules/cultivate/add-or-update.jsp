                        ...
                        <form id="addOrUpdateForm">
                            ...
                            <!-- 当前表的字段 -->
                                ...
                                <input id="yonghuId" name="yonghuId" type="hidden">
                                   <div class="form-group col-md-6">
                                       <label>培训开始时间</label>
                                       <input id="onTime-input" name="onTime" class="form-control" size="20" type="date">
                                   </div>
                                   <div class="form-group col-md-6">
                                       <label>培训结束时间</label>
                                       <input id="downTime-input" name="downTime" class="form-control" size="20" type="date">
                                   </div>
...
<script>
    ...
    function submit() {
        if (validform() == true && compare() == true) {
            ...
            httpJson("cultivate/" + urlParam, "POST", data, (res) => {
                if(res.code == 0)
                {
                    window.sessionStorage.removeItem('addcultivate');
    ...
    // 查询列表
        ...
        function yonghuSelect() {
            //填充下拉框选项
            http("yonghu/page?page=1&limit=100&sort=&order=&yonghuTypes=1", "GET", {}, (res) => {
                if(res.code == 0){
    ...
    function setContent() {
        ...
        <!-- 当前表的填充富文本框 -->
        if (ruleForm.cultivateContent != null && ruleForm.cultivateContent != 'null' && ruleForm.cultivateContent != '') {
            var mes = '' + ruleForm.cultivateContent;
    ...
    //数字检查
        ...
        function cultivateContentChickValue(e){
            var this_val = e.value || 0;
            if(this_val == 0){
                e.value = "";
                alert("0不允许输入");
    ...
    // 获取当前详情
    function getDetails() {
        var addcultivate = window.sessionStorage.getItem("addcultivate");
        if (addcultivate != null && addcultivate != "" && addcultivate != "null") {
            window.sessionStorage.removeItem('addcultivate');
            //注册表单验证
            $(validform());
            $('#submitBtn').text('新增');
            ...
            window.sessionStorage.removeItem('updateId');
            http("cultivate/info/" + updateId, "GET", {}, (res) => {
                if(res.code == 0)
                {
                    ruleForm = res.data
                    // 是/否下拉框回显
    ...
    <!--  当前表的数据回显  -->
        ...
        $("#dayNumber").val(ruleForm.dayNumber);
    ...
    function yonghuDataBind(){
        ...
        $("#name").val(ruleForm.yonghuName);
        $("#phone").val(ruleForm.yonghuPhone);
        $("#bumenValue").val(ruleForm.bumenValue);
        $("#zhiweiValue").val(ruleForm.zhiweiValue);
        $("#idNumber").val(ruleForm.yonghuIdNumber);
