                        ...
                        <form id="addOrUpdateForm">
                            ...
                            <!-- 当前表的字段 -->
                                    ...
                                    <div class="form-group col-md-6 aaaaaa">
                                        <label>权限</label>
                                        <select id="roleTypesSelect" name="roleTypes" class="form-control">
                                        </select>
                                    </div>
...
<script>
    ...
    <!-- 初始化当前表的下拉框 -->
        ...
        function initializationBumentypesSelect(){
            ...
            if(bumenTypesSelect != null && bumenTypesOptions != null  && bumenTypesOptions.length > 0 ){
                bumenTypesSelect.add(new Option("",""));
        ...
        function initializationRoletypesSelect(){
            var roleTypesSelect = document.getElementById('roleTypesSelect');
            if(roleTypesSelect != null && roleTypesOptions != null  && roleTypesOptions.length > 0 ){
                for (var i = 0; i < roleTypesOptions.length; i++) {
                    roleTypesSelect.add(new Option(roleTypesOptions[i].indexName,roleTypesOptions[i].codeIndex));
                }
            }
        }
    ...
    function setSelectOption() {
        ...
        if(roleTypesSelect != null && roleTypesOptions != null  && roleTypesOptions.length > 0 ) {
            for (var i = 0; i < roleTypesOptions.length; i++) {
                if (roleTypesOptions[i].codeIndex == ruleForm.roleTypes) {//下拉框value对比,如果一致就赋值汉字
                        roleTypesSelect.options[i].selected = true;
                }
            }
        }
        <!--  级联表的下拉框回显  -->
    ...
    function readonly() {
        if (window.sessionStorage.getItem('role') == '管理员') {
            ...
            $('#roleTypesSelect').attr('style', 'pointer-events: none;');

        }else if(window.sessionStorage.getItem('role') == '部门主管'){
            $('#bumenTypesSelect').attr('style', 'pointer-events: none;');
            $('#roleTypesSelect').attr('style', 'pointer-events: none;');

        }else if(window.sessionStorage.getItem('role') == '总经理'){
            $('#roleTypesSelect').attr('style', 'pointer-events: none;');
